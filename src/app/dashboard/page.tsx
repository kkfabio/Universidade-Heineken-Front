...
export default function Dashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAF9] text-[#1A1A1A]">
      <Sidebar />
      
      <div className="flex-1 flex flex-col">
        <Navbar />

        {/* O pl-24 garante que o conteúdo respire e vá para a direita */}
        <main className="py-12 pl-24 pr-12 max-w-[1500px] w-full mx-auto lg:mx-0">
                  ...
                  {cursos.map((c) => (
                    <div key={c.id} className={c.status ? 'opacity-100' : 'opacity-40'}>
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className={`w-9 h-4.5 rounded-full relative cursor-pointer transition-colors ${c.status ? 'bg-[#00b140]' : 'bg-slate-200'}`}>
                          <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full transition-all ${c.status ? 'right-0.5' : 'left-0.5'}`}></div>
                        </div>
                      </div>
